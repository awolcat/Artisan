...
export default function Contract(props) {
    ...
    async function getIdentity() {
        const idUrl = 'http://' + window.location.hostname + ':80/current_user';
    ...
    const handleSubmit = async (event) => {
        ...
        async function submitContract() {
            const response = await fetch('http://' + window.location.hostname + ':80/api/v1/contracts', {
                method: 'POST',
            ...
            if (response.status === 200) {
                ...
                const bookingResponse = await fetch('http://' + window.location.hostname + ':80/api/v1/bookings', {
                    method: 'POST',
                    headers: {'Content-Type': 'application/json',},
                    body: JSON.stringify({
