...
export default function Jobs(props) {
    ...
    async function getOpenContracts() {
        ...
        try {
            const url = 'http://' + window.location.hostname + '/api/v1/contracts'
    ...
    async function bookJob(contract) {
        try {
            ...
            const contract_url = 'http://' + window.location.hostname + '/api/v1/contracts/' + contract.id;
            const bookingResponse = await fetch('http://' + window.location.hostname + '/api/v1/bookings', {
            method: 'POST',
    ...
    async function getServices() {
        try {
            const url = 'http://' + window.location.hostname + '/api/v1/services/';
            const response = await fetch(url);
            const data = await response.json();
            setServices(data);
        }
