...
export default function Jobs(props) {
    ...
    async function getOpenContracts() {
        ...
        try {
            const url = 'http://127.0.0.1/api/v1/contracts'
    ...
    async function bookJob(contract) {
        try {
            ...
            const contract_url = 'http://127.0.0.1/api/v1/contracts/' + contract.id;
            const bookingResponse = await fetch('http://127.0.0.1/api/v1/bookings', {
            method: 'POST',
    ...
    async function getServices() {
        try {
            const url = 'http://127.0.0.1/api/v1/services/';
            const response = await fetch(url);
            const data = await response.json();
            setServices(data);
        }
